  ...
  return (
    <>
      ...
      <AnimatePresence>
        {showGreeting && !greetingDismissed && (
          <motion.div
            ...
            className="fixed bottom-24 right-6 z-40 max-w-xs font-sans"
          >
            <div
              className="bg-white rounded-2xl shadow-2xl border border-gray-200 p-4 cursor-pointer hover:shadow-3xl transition-shadow duration-300 relative font-sans"
              onClick={handleGreetingClick}
            >
              <div className="flex items-start gap-3">
                <div className="w-8 h-8 bg-gradient-to-r from-[#22396b] to-[#3d3dff] rounded-full flex items-center justify-center flex-shrink-0 font-sans">
                ...
                <div className="flex-1 pr-6">
                  ...
                  <p className="text-xs text-gray-500 mt-1 font-sans">Click to chat with me!</p>
                </div>
                <button
                  onClick={handleGreetingDismiss}
                  className="absolute top-2 right-2 w-5 h-5 text-gray-400 hover:text-gray-600 flex items-center justify-center rounded-full hover:bg-gray-100 transition-colors font-sans"
                >
                  <X className="w-3 h-3" />
                </button>
              </div>
      ...
      <motion.button
        ...
        className={`fixed bottom-6 right-6 z-50 w-14 h-14 bg-gradient-to-r from-[#22396b] to-[#3d3dff] text-white rounded-full shadow-xl hover:shadow-2xl transition-all duration-300 ${isOpen ? 'hidden' : 'flex'} items-center justify-center font-sans`}
      >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-6 right-6 z-50 w-96 max-w-[calc(100vw-2rem)] h-[500px] bg-white rounded-2xl shadow-2xl border border-gray-200 overflow-hidden font-sans"
          >
            {/* Header */}
            <div className="bg-gradient-to-r from-[#22396b] to-[#3d3dff] text-white p-4 flex items-center justify-between font-sans">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 bg-white/20 rounded-full flex items-center justify-center font-sans">
                  <Bot className="w-5 h-5" />
                </div>
                <div>
                  <h3 className="font-semibold font-sans">Trinity</h3>
                  <p className="text-xs text-white/80 font-sans">Digital Transformation Assistant</p>
                </div>
              </div>
              <button
                onClick={() => setIsOpen(false)}
                className="w-8 h-8 hover:bg-white/20 rounded-full flex items-center justify-center transition-colors font-sans"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            {/* Messages */}
            <div className="flex-1 p-4 overflow-y-auto max-h-[320px] space-y-4 font-sans">
              {messages.map((message) => (
                <motion.div
                  key={message.id}
                  initial={{ opacity: 0, y: 10 }}
                  animate={{ opacity: 1, y: 0 }}
                  className={`flex gap-2 ${message.sender === 'user' ? 'flex-row-reverse' : 'flex-row'} font-sans`}
                >
              ...
              {isTyping && (
                ...
                >
                  <div className="w-8 h-8 rounded-full bg-gray-100 text-[#22396b] flex items-center justify-center font-sans">
                    <Bot className="w-4 h-4" />
                  </div>
                  <div className="bg-gray-100 p-3 rounded-2xl rounded-bl-md font-sans">
                ...
                <div className="flex flex-wrap gap-2">
                  {quickActions.map((action, index) => (
                    <motion.button
                      ...
                      className="px-3 py-1 text-xs bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-full transition-colors font-sans"
                    >
                      {action}
                    </motion.button>
                  ))}
            ...
            <div className="p-4 border-t border-gray-200 font-sans">
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-4 py-2 bg-gray-100 rounded-full text-sm focus:outline-none focus:ring-2 focus:ring-[#3d3dff] focus:bg-white transition-all font-sans"
                  disabled={isTyping}
                />
                <button
                  onClick={() => handleSendMessage(inputText)}
                  disabled={!inputText.trim() || isTyping}
                  className="w-10 h-10 bg-gradient-to-r from-[#22396b] to-[#3d3dff] text-white rounded-full flex items-center justify-center hover:shadow-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed font-sans"
                >
